...
class UserToken extends Token
{
    ...
    public function get(){
       $result = curl_get($this->wxLoginUrl);//http请求，获取API地址的第一步，获取微信API返回的json格式数据
       $wxResult = json_decode($result,true);//把json格式转换为数组
       if(empty($wxResult)){//code码非法的时候$wxResult视为空
           throw new Exception("微信内部错误，获取session_key和openid异常");//服务器内部错误不能把错误原因传到客户端但这要记录日志
       }else{
           $loginFail = array_key_exists("errcode",$wxResult);//有错误的时候微信会返回errorcode码
           if($loginFail){
    ...
    private function saveToCache($cacheValue){
        ...
        /*
         * cache($name,$value="",$option=null)
         * $name:传入是数组，则表示进行缓存初始化；若为字符串则表示缓存赋值，获取或删除操作
         * $value:要设置的缓存值，若为null则表示删除缓存，默认为空字符串
         * */
